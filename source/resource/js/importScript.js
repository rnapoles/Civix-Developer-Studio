//
// This file is part of Honey importScript
//
// Copyright (c) 2015 Torben Haase
//
// Honey importScript is free software: you can redistribute it and/or modify it
...
// SOFTWARE.
//
// You should have received a copy of the MIT License along with Honey importScript.
...
//      afterwards!
    ...
    var importScriptPath = self.Honey&&self.Honey.importScriptPath!==undefined ? self.Honey.importScriptPath.slice(0) : ['./'];
    var importScriptCompiler = self.Honey&&self.Honey.importScriptCompiler!==undefined ? self.Honey.importScriptCompiler : null;

// NOTE Parse module root paths
    var base = [location.origin, location.href.substr(0, location.href.lastIndexOf("/")+1)];
    for (var i=0; i<importScriptPath.length; i++) {
        parser.href = (importScriptPath[i][0]=="."?base[1]:base[0])+importScriptPath[i];
        importScriptPath[i] = parser.href;
    }

// NOTE Add preloaded modules to cache
    for (var id in (self.Honey && self.Honey.importScriptPreloaded))
        cache['$'+resolve(id).id] = self.Honey.importScriptPreloaded[id].toString();

// NOTE Add module overrides to cache
    for (var id in (self.Honey && self.Honey.importScriptOverrides))
        cache['$'+resolve(id).id] = self.Honey.importScriptOverrides[id];
...
//      module has been loaded.
    ...
    function importScript(identifier, callback, compiler) {
        ...
        if (identifier instanceof Array) {
            ...
            for (var index = 0; index < identifier.length; index++) {
                (function(id, i) {
                    modules.push(importScript(id, callback&&function(mod) {
                        modules[i] = mod;
        ...
        compiler = compiler!==undefined ? compiler : importScriptCompiler;
...
//      `fetch` to load a module.
    ...
    function resolve(identifier) {
        ...
        var root = m[2] ? importScriptPath[p[1]?parseInt(p[1]):0] : importScriptPath[m[1]?parseInt(m[1]):0];
...
// INFO Exporting importScript to global scope

    if (self.importScript !== undefined)
        throw new Error("Honey: '\'importScript\' already defined in global scope");

    try {
        Object.defineProperty(self, 'importScript', {'value':importScript});
        Object.defineProperty(self.importScript, 'resolve', {'value':resolve});
        Object.defineProperty(self.importScript, 'path', {'get':function(){return importScriptPath.slice(0);}});
    }
    catch (e) {
        ...
        self.importScript.resolve = resolve;
        self.importScript.path = importScriptPath.slice(0);
    }
...
//      in strict mode, too.
    ...
    function /*load*/(module/*, cache, pwd, source*/) {
        ...
        if (typeof module.id !== 'string')
            for (id in module)
                arguments[1]['$'+importScript.resolve(id).id] = module[id].toString();
        arguments[2].shift();
    }
